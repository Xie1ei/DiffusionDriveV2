...
class NATLayer(nn.Module):
    def __init__(
        ...
        # 替换 NeighborhoodAttention1D 为标准的 TransformerEncoderLayer
        # 注意：kernel_size 和 dilation 参数在标准注意力中不使用，所以忽略它们
    ...
    def forward(self, x):
        shortcut = x
        x = self.norm1(x)
        # TransformerEncoderLayer 的 forward 方法需要输入 x
        x = self.attn(x)
...
#     model = NATSequenceEncoder(in_chans=9)
#     total_params = sum(p.numel() for p in model.parameters())
#     print(f"总参数数：{total_params}") 
#     inp = torch.randn(8, 9, 20) # bs*A, dim, T
#     with torch.no_grad():
#         out = model(inp)
#         print(out.shape)
